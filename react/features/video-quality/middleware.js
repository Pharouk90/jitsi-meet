...
StateListenerRegistry.register(
    ...
    /* listener */ ({ displayTileView, reducedUI, thumbnailHeight }, { dispatch, getState }) => {
        ...
        let maxRecvFrameHeight = VIDEO_QUALITY_LEVELS.HIGH;

        if (reducedUI) {
            maxRecvFrameHeight = VIDEO_QUALITY_LEVELS.LOW;
        } else if (displayTileView && !Number.isNaN(thumbnailHeight)) {
            maxRecvFrameHeight = getNearestReceiverVideoQualityLevel(thumbnailHeight);
        }

        if (maxReceiverVideoQuality !== maxRecvFrameHeight) {
            dispatch(setMaxReceiverVideoQuality(maxRecvFrameHeight));
        }
    }, {
        deepEquals: true
    });
